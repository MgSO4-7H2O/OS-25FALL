...
void task_init() {
    ...
    for (int i = 1; i < NR_TASKS; ++i) {
        ...
        // 分配独立物理内存
        uint64_t alloc_va = (uint64_t)alloc_pages(uapp_pages);
        memcpy((void *)alloc_va, (void *)_sramdisk, uapp_size);
        uint64_t pa = alloc_va - (uint64_t)PA2VA_OFFSET;

        // 建立页表映射关系
        create_mapping(pg, USER_START, pa, uapp_size, PTE_V | PTE_R | PTE_W | PTE_X | PTE_U);
