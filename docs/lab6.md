...
!!! abstract "ber，话又说回来，还是得相信自己是最优秀的那批人"

!!! tip "DDL"

    寿黎但班DDL: 2026.01.11 23:55
...
!!! note "本次实验中涉及 VirtIO 的部分已经为大家实现好了，同学们只需要调用相关函数，使用 VirtIO-blk 相关的接口，在虚拟磁盘设备上进行块的读取与写入即可，对 VirtIO 的具体交互方式感兴趣的同学，可以阅读[rCoreOS](https://rcore-os.cn/rCore-Tutorial-Book-v3/chapter9/2device-driver-3.html)的文档"

### MBR

主引导记录（Master Boot Record，MBR），又叫做主引导扇区，是计算机开机后访问硬盘时所必须要读取的首个扇区，它位于硬盘的第0个扇区。在深入讨论主引导扇区内部结构的时候，有时也将其开头的 446 字节内容指为“主引导记录”（MBR）,BIOS会使用这 446 字节的代码启动操作系统 ,其后是 4 个 16 字节的“磁盘分区表”（DPT），以及 2 字节的结束标志（55AA）。本实验只需要解析DPT的内容，根据DPT找到FAT分区的起始与结束相关信息即可
