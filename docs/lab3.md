...
验收环节将从以下问题中挑选gdb操作作为考核标准：
<!-- 1、在中断入口处设置断点，记录保存前的栈内存内容，再在执行保存上下文的指令后观察栈上内容作比较，并将其与寄存器内容对应 -->

1、在create_mapping完成后，根据C的调试内容，找到被修改的PTE项，说明这段虚存映射及其权限设置

2、在启用MMU的初始页表映射后，找到当前运行中PC指向的内存值，以及高地址处的内存值，验证两者的内容是否相同

3、在三级页表建立完成后，根据satp找到页表根节点所在的内存页，并根据页表的PTE内容找出第二级页表的地址及其内容

4、在不同的虚存进程上下文中，通过dummy断点进入调试后，栈指针有什么不一样的地方，这个栈页面底部的task_struct呢？
