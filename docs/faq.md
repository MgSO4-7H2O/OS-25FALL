...
从lab1开始，实验提交时需要make clean后的代码压缩包和实验报告的两个文件；其中实验报告要求上传 **pdf文件** 和包含文字内容的 **txt文件** ， pdf 需要包含以下内容：
...
建议使用markdown、LaTeX(overleaf)编写报告，不建议使用word编写，更不能提交word文件
...
- 能在解压后的文件夹下使用make run命令，生成vmlinux并启动qemu产生输出
- 在压缩前记得清理中间文件，需要使用跨平台支持较好的zip做压缩处理

按照上面的要求提交三个实验文件，保护一下卑微助教🙏~~不这么做容易导致改报告的TA对着一排100MB+的rar文件发愁~~
